...
public class EventResource extends RestResource<Event> {
	...
	protected ResponseEntity<String> sync(HttpServletRequest request) {
		...
		try {
			...
			if (team != null || providerId != null || locationId != null || baseEntityId != null || eventType != null) {
				...
				if (!events.isEmpty()) {
					for (Event event : events) {
						if (event.getBaseEntityId() != null && !event.getBaseEntityId().isEmpty() && !clientIds
								.contains(event.getBaseEntityId())) {
							clientIds.add(event.getBaseEntityId());
						}
					}
					for (int i = 0; i < clientIds.size(); i = i + CLIENTS_FETCH_BATCH_SIZE) {
						int end = i + CLIENTS_FETCH_BATCH_SIZE < clientIds.size() ?
								i + CLIENTS_FETCH_BATCH_SIZE :
								clientIds.size();
			...
			JsonArray clientsArray = (JsonArray) gson.toJsonTree(clients, new TypeToken<List<Client>>() {
	...
	public ResponseEntity<HttpStatus> save(@RequestBody String data) {
		try {
			...
			if (syncData.has("clients")) {
				...
				ArrayList<Client> clients = (ArrayList<Client>) gson
						.fromJson(syncData.getString("clients"), new TypeToken<ArrayList<Client>>() {
				...
				for (Client client : clients) {
					try {
						...
						logger.error("Client" + client.getBaseEntityId() == null ?
								"" :
								client.getBaseEntityId() + " failed to sync", e);
					}
				}

			}
			if (syncData.has("events")) {
				ArrayList<Event> events = (ArrayList<Event>) gson
						.fromJson(syncData.getString("events"), new TypeToken<ArrayList<Event>>() {
